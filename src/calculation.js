(function calculate() {
  ...
  frame.on('ready', () => {
    ...
    let rearCar = new Rectangle(rearCarLength, rearCarWidth, red, black, 1).pos(
      rearCarFromLeft,
      rearCarY,
      LEFT,
      TOP,
    ...
    let parkingSpace = new Rectangle(parkingSpaceLength, frontCarWidth, 0).pos(
      parkingSpaceFromLeft,
      0,
      LEFT,
      BOTTOM,
    );

    let frontCar = new Rectangle(
      frontCarLength,
      frontCarWidth,
      red,
      black,
    ...
    let carRearAxle = new Line({
      ...
      points: [
        [
          carX - carLength / 2 + carRearOverhang,
          carY + carWidth / 2 - carSideOverhang,
        ],
        [
          carX - carLength / 2 + carRearOverhang,
          carY - carWidth / 2 + carSideOverhang,
    ...
    const turnCircle = new Circle(
      carTurnCenterFromCarCenter,
      null,
      'red',
      1,
    ).pos({
      x: carX - carLength / 2 + carRearOverhang,
      y: carY - carTurnCenterFromCarCenter,
      regX: true,
      regY: true,
    });

    const turnCircle2 = new Circle(carTurningCircle, null, 'red', 1).pos({
      x: carX - carLength / 2 + carRearOverhang,
      y: carY - carTurnCenterFromCarCenter,
      regX: true,
      regY: true,
    ...
    function step(timestamp) {
      ...
      if (elapsed <= 10000) {
        carContainer.rot(
          -count / 100,
          carX - carLength / 2 + carRearOverhang,
          carY - carTurnCenterFromCarCenter,
        );
      }
      if (elapsed > 10000) {
